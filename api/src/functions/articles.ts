...
export async function getArticles(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  context.log('GET /api/articles');

  const slug = request.params.slug;
  if (slug) {
    const article = articles.find(item => item.slug === slug);
    if (!article) {
      return json({ message: 'Article not found' }, 404);
    }
    return json(article);
  }

  return json(articles.map(({ content, ...summary }) => summary));
}
